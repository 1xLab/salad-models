...
TARGET_FILE="$MODEL_DIR/Qwen3.6-35B-A3B-UD-Q4_K_XL.gguf"

mkdir -p "$MODEL_DIR"

if [[ ! -s "$MODEL_FILE" ]]; then
    echo "=== Downloading Qwen 3.6 35B GGUF from Backblaze B2 via rclone (8 parallel streams) ==="
    ...
    rclone copy "b2:${S3_BUCKET:-devopsprojects}/models/Qwen3.6-35B-A3B-UD-Q4_K_XL.gguf" "$MODEL_DIR/" \
